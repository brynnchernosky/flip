                             ...
                             std::vector<Eigen::Vector3i> &faces)
{
  ...
  for (unsigned int i = 0; i < shapes[0].mesh.indices.size(); i += 3) {
    faces.push_back(Vector3i(shapes[0].mesh.indices[i].vertex_index,
  ...
  for (unsigned int i = 0; i < shapes[0].mesh.indices.size(); i++) {
    ...
    if (verts.find(vind) == verts.end()) {
      vertices[shapes[0].mesh.indices[i].vertex_index] = Vector3f(attrib.vertices[vind],
                                                                  attrib.vertices[vind + 1],
                                                                  attrib.vertices[vind + 2]);
      verts.emplace(vind);
    }
  }
